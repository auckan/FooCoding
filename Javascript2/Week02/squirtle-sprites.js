...
const displayImages = (objectData) => {
  for (let key in objectData.sprites) {
    const spritesImage = objectData.sprites[key];
    console.log(spritesImage);
    if (spritesImage !== null) {
      const img = document.createElement("img");
      container.appendChild(img);
      img.setAttribute("src", spritesImage);
    }
  }
};

displayImages(pokemonData);
